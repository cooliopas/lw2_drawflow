export default class Drawflow {
    constructor(container, render = null, parent = null) {
        ...
        this.curvature = 0.2;
        this.reroute_curvature_start_end = 0.2;
        this.reroute_curvature = 0.2;
            ...
            if (ele_last.classList[0] === 'input' || (this.force_first_input && (ele_last.closest('.drawflow_content_node') != null || ele_last.classList[0] === 'drawflow-node'))) {
                ...
                if (output_id !== input_id && input_class !== false) {
                    ...
                    if (this.container.querySelectorAll('.connection.node_in_' + input_id + '.node_out_' + output_id + '.' + output_class + '.' + input_class).length === 0) {
                        ...
                        var outputElementColor = this.drawflow.drawflow[this.module].data[id_output].data.color;
                        if (outputElementColor) {
                            var mainPath = this.connection_ele.querySelector('.main-path');
                            mainPath.setAttribute('style', `stroke: ${outputElementColor} !important`);
                        }
    ...
    updateConnectionNodes(id) {
        ...
        Object.keys(elemsOut).map(function (item) {
            if (elemsOut[item].querySelector('.point') === null) {
                ...
                var outputElement = elemtsearchId_out.querySelector('.freeFormplotPointTitle');
                if (outputElement) {
                    var color = window.getComputedStyle(outputElement).backgroundColor;
                    if (color) {
                        elemsOut[item].children[0].style.stroke = color;

                        var inputVal = elemtsearchId_out.querySelectorAll('.' + `input_${elemsOut[item].classList[3].split('_')[1]}`)[0];

                        elemtsearchOut.style.background = color;
                        inputVal.style.background = color;
                    }
                }

                var inputElement = document.querySelector(`#${elemsOut[item].classList[1].split('node_in_').slice(1).join('')}`);
                if (inputElement) {
                    var inputNode = inputElement.querySelector(`.${elemsOut[item].classList[4]}`);
                    var outputNode = inputElement.querySelector(`.output_${elemsOut[item].classList[4].split('_')[1]}`);
                    if (inputNode && outputNode) {
                        inputNode.style.background = color;
                        outputNode.style.background = color;
                    }
                }
            } else {
    ...
    removeConnection() {
        if (this.connection_selected != null) {
            ...
            var output_node = document.querySelector(`#${'node-' + listclass[2].slice(14)}`);
            if (output_node) {
                var output = output_node.querySelector(`.${listclass[3]}`);
                if (output) {
                    output.removeAttribute('style');
                }
            }
            var input_node = document.querySelector(`#${'node-' + listclass[1].slice(13)}`);
            if (input_node) {
                var input = input_node.querySelector(`.${listclass[4]}`);
                if (input) {
