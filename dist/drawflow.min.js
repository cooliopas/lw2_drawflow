export default class Drawflow {
    constructor(container, render = null, parent = null) {
        ...
        this.curvature = 0.1;
        this.reroute_curvature_start_end = 0.1;
        this.reroute_curvature = 0.1;
    ...
    addLabelTextImport(conn, labelText, output_id, output_class, input_id, input_class) {
        const connIndex = this.drawflow.drawflow[this.module].data[output_id].outputs[output_class].connections.findIndex(
            (conn) => conn.node == input_id && conn.output == input_class,
        );

        // Only proceed if we have a valid connection and labelText
        if (conn != null && connIndex != null && connIndex != -1 && labelText) {
            this.drawflow.drawflow[this.module].data[output_id].outputs[output_class].connections[connIndex].labelText = labelText;
            const newid = [conn.classList].join().replace(/\s/g, '');
            conn.childNodes[0].id = newid;

            // Create label group
            const labelGroup = document.createElementNS(conn.namespaceURI, 'g');
            labelGroup.setAttribute('class', 'connection-label');
            labelGroup.style.pointerEvents = 'none';
            labelGroup.style.zIndex = '9999';

            // Create background rectangle
            const bgRect = document.createElementNS(conn.namespaceURI, 'rect');
            bgRect.setAttribute('x', '-12');
            ...
            bgRect.setAttribute('stroke', 'var(--q-secondary)');
            bgRect.setAttribute('class', 'connection-label-bg');
            bgRect.style.filter = 'drop-shadow(0px 2px 3px rgba(var(--q-secondary),0.1))';
            bgRect.style.pointerEvents = 'all';

            // Create icon
            const iconSvg = document.createElementNS(conn.namespaceURI, 'svg');
            iconSvg.setAttribute('width', '14');
            iconSvg.setAttribute('height', '12');
            iconSvg.setAttribute('viewBox', '0 0 14 12');
            iconSvg.setAttribute('x', '-7');
            iconSvg.setAttribute('y', '-6');
            iconSvg.setAttribute('class', 'connection-label-icon');
            iconSvg.style.pointerEvents = 'all';

            const iconPath = document.createElementNS(conn.namespaceURI, 'path');
            iconPath.setAttribute(
                'd',
                'M13 11C13 9.65161 11.8867 7.73032 10.3333 7.30516M9 11C9 9.28968 7.20933 7.12903 5 7.12903C2.79067 7.12903 1 9.28968 1 11M9 4.87097C9.53043 4.87097 10.0391 4.66705 10.4142 4.30408C10.7893 3.9411 11 3.44881 11 2.93548C11 2.42216 10.7893 1.92986 10.4142 1.56689C10.0391 1.20392 9.53043 1 9 1M7 2.93548C7 3.44881 6.78929 3.9411 6.41421 4.30408C6.03914 4.66705 5.53043 4.87097 5 4.87097C4.46957 4.87097 3.96086 4.66705 3.58579 4.30408C3.21071 3.9411 3 3.44881 3 2.93548C3 2.42216 3.21071 1.92986 3.58579 1.56689C3.96086 1.20392 4.46957 1 5 1C5.53043 1 6.03914 1.20392 6.41421 1.56689C6.78929 1.92986 7 2.42216 7 2.93548Z',
            );
            iconPath.setAttribute('stroke', 'var(--q-secondary)');
            ...
            const text = document.createElementNS(conn.namespaceURI, 'text');
            ...
            // Create hidden text with textPath for auto-updates
            const hiddenText = document.createElementNS(conn.namespaceURI, 'text');
            hiddenText.style.opacity = '0';
            hiddenText.style.position = 'absolute';
            hiddenText.style.pointerEvents = 'none';
            hiddenText.setAttribute('aria-hidden', 'true');

            const textPath = document.createElementNS(conn.namespaceURI, 'textPath');
            textPath.setAttribute('href', `#${newid}`);
            ...
            textPath.style.pointerEvents = 'none';

            // Add hover effects
            const addHoverEffect = () => {
                const textWidth = text.getBBox().width;
                bgRect.setAttribute('width', `${textWidth + 40}`);
                text.setAttribute('opacity', '1');
                bgRect.setAttribute('fill', '#F5F8FF');
            };

            const removeHoverEffect = () => {
                if (!textPath.contentEditable || textPath.contentEditable === 'false') {
                    bgRect.setAttribute('width', '24');
                    text.setAttribute('opacity', '0');
                    bgRect.setAttribute('fill', 'white');
                }
            ...
            // Create a hover area that only includes the icon and its background
            const hoverArea = document.createElementNS(conn.namespaceURI, 'g');
            hoverArea.style.pointerEvents = 'all';
            hoverArea.appendChild(bgRect);
            hoverArea.appendChild(iconSvg);

            // Add hover listeners only to the hover area
            hoverArea.addEventListener('mouseenter', addHoverEffect);
            ...
            const existingLabel = conn.querySelector('g');
            if (existingLabel) {
                existingLabel.remove();
            }

            // Append elements in the correct order
            labelGroup.appendChild(hoverArea);
            labelGroup.appendChild(hiddenText);
            labelGroup.appendChild(text);

            conn.appendChild(labelGroup);

            // Add position observer
            const observer = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    if (mutation.type === 'attributes' && mutation.attributeName === 'd') {
                        const mainPath = conn.childNodes[0];
                        const newPathD = mainPath.getAttribute('d');
                        const newMatches = newPathD.match(
                            /M\s*([0-9.-]+)\s*([0-9.-]+)\s*C\s*([0-9.-]+)\s*([0-9.-]+)\s*([0-9.-]+)\s*([0-9.-]+)\s*([0-9.-]+)\s*([0-9.-]+)/,
                        );

                        if (newMatches) {
                            const newStartX = parseFloat(newMatches[1]);
                            const newStartY = parseFloat(newMatches[2]);
                            const newEndX = parseFloat(newMatches[7]);
                            const newEndY = parseFloat(newMatches[8]);

                            const newMidX = (newStartX + newEndX) / 2;
                            const newMidY = (newStartY + newEndY) / 2;

                            labelGroup.setAttribute('transform', `translate(${-9999 + newMidX}, ${-9999 + newMidY})`);
                        }
                    }
                });
            });

            observer.observe(conn.childNodes[0], {
                attributes: true,
                attributeFilter: ['d'],
            });
        }
    ...
    addLabelText(labelText, output_id, output_class, input_id, input_class) {
        ...
        if (bgPath != null && connIndex != null && connIndex != -1 && labelText) {
            this.drawflow.drawflow[this.module].data[output_id].outputs[output_class].connections[connIndex].labelText = labelText;

            const mainPath = bgPath.querySelector('.main-path');
            const newid = [bgPath.classList].join().replace(/\s/g, '');
            mainPath.id = newid;

            const pathD = mainPath.getAttribute('d');
            const matches = pathD.match(/M\s*([0-9.-]+)\s*([0-9.-]+)\s*C\s*([0-9.-]+)\s*([0-9.-]+)\s*([0-9.-]+)\s*([0-9.-]+)\s*([0-9.-]+)\s*([0-9.-]+)/);

            if (matches) {
                const startX = parseFloat(matches[1]);
                const startY = parseFloat(matches[2]);
                const endX = parseFloat(matches[7]);
                const endY = parseFloat(matches[8]);

                const midX = (startX + endX) / 2;
                const midY = (startY + endY) / 2;

                const labelGroup = document.createElementNS(bgPath.namespaceURI, 'g');
                labelGroup.setAttribute('transform', `translate(${-9999 + midX}, ${-9999 + midY})`);
                labelGroup.setAttribute('class', 'connection-label');
                labelGroup.style.pointerEvents = 'none'; // Changed to none

                // Add data attributes for editing
                labelGroup.dataset.id_output = output_id;
                labelGroup.dataset.output_class = output_class;
                labelGroup.dataset.id_input = input_id;
                labelGroup.dataset.id_class = input_class;

                // Create background rectangle
                const bgRect = document.createElementNS(bgPath.namespaceURI, 'rect');
                bgRect.setAttribute('x', '-12');
                bgRect.setAttribute('y', '-12');
                bgRect.setAttribute('width', '24');
                bgRect.setAttribute('height', '24');
                ...
                bgRect.setAttribute('stroke', 'var(--q-secondary)');
                bgRect.setAttribute('class', 'connection-label-bg');
                bgRect.style.filter = 'drop-shadow(0px 2px 3px rgba(0,0,0,0.1))';

                // Create icon
                const iconSvg = document.createElementNS(bgPath.namespaceURI, 'svg');
                iconSvg.setAttribute('width', '14');
                iconSvg.setAttribute('height', '12');
                iconSvg.setAttribute('viewBox', '0 0 14 12');
                iconSvg.setAttribute('x', '-7');
                ...
                const iconPath = document.createElementNS(bgPath.namespaceURI, 'path');
                iconPath.setAttribute(
                    'd',
                    'M13 11C13 9.65161 11.8867 7.73032 10.3333 7.30516M9 11C9 9.28968 7.20933 7.12903 5 7.12903C2.79067 7.12903 1 9.28968 1 11M9 4.87097C9.53043 4.87097 10.0391 4.66705 10.4142 4.30408C10.7893 3.9411 11 3.44881 11 2.93548C11 2.42216 10.7893 1.92986 10.4142 1.56689C10.0391 1.20392 9.53043 1 9 1M7 2.93548C7 3.44881 6.78929 3.9411 6.41421 4.30408C6.03914 4.66705 5.53043 4.87097 5 4.87097C4.46957 4.87097 3.96086 4.66705 3.58579 4.30408C3.21071 3.9411 3 3.44881 3 2.93548C3 2.42216 3.21071 1.92986 3.58579 1.56689C3.96086 1.20392 4.46957 1 5 1C5.53043 1 6.03914 1.20392 6.41421 1.56689C6.78929 1.92986 7 2.42216 7 2.93548Z',
                );
                ...
                // Create text
                const text = document.createElementNS(bgPath.namespaceURI, 'text');
                text.setAttribute('x', '14');
                text.setAttribute('y', '0');
                text.setAttribute('fill', '#3C3C3C');
                text.setAttribute('font-size', '12');
                ...
                text.setAttribute('opacity', '0');
                text.setAttribute('dominant-baseline', 'middle');
                text.style.pointerEvents = 'none'; // Disable pointer events on text
                text.textContent = labelText;
                ...
                const addHoverEffect = () => {
                    ...
                    bgRect.setAttribute('width', `${textWidth + 40}`);
                    text.setAttribute('opacity', '1');
                    bgRect.setAttribute('fill', 'var(--q-secondary)');
                };

                const removeHoverEffect = () => {
                    if (!text.contentEditable || text.contentEditable === 'false') {
                        bgRect.setAttribute('width', '24');
                        text.setAttribute('opacity', '0');
                        bgRect.setAttribute('fill', 'white');
                    }
                ...
                // Create a hover area that only includes the icon and its background
                const hoverArea = document.createElementNS(bgPath.namespaceURI, 'g');
                hoverArea.style.pointerEvents = 'all'; // Enable pointer events only for this group
                ...
                // Remove existing label if any
                const existingLabel = bgPath.querySelector('g');
                if (existingLabel) {
                    existingLabel.remove();
                }
                ...
                hoverArea.appendChild(bgRect);
                hoverArea.appendChild(iconSvg);
                labelGroup.appendChild(hoverArea); // Add hover area first
                labelGroup.appendChild(text);

                bgPath.appendChild(labelGroup);
    ...
    addNode(name, num_in, num_out, ele_pos_x, ele_pos_y, classoverride, data, html, typenode = false) {
        ...
        const deleteNode = document.createElement('div');
        (deleteNode.innerHTML = '<i class="fa-thin fa-x"></i>'),
            deleteNode.setAttribute('id', 'node-' + newNodeId + '-delete'),
            deleteNode.setAttribute('data-id', newNodeId),
            deleteNode.classList.add('drawflow-delete-node');

        const icon = document.createElement('div');

        if (data.hasOwnProperty('type')) {
            icon.classList.add('icon-with-bg');
            if (data.type == 'outline') {
                icon.innerHTML =
                    '<svg width="29" height="27" viewBox="0 -0.5 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 4H14" stroke="#fa9dae" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 8H14" stroke="#fa9dae" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 12H14" stroke="#fa9dae" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M3.25 4.75C3.66421 4.75 4 4.41421 4 4C4 3.58579 3.66421 3.25 3.25 3.25C2.83579 3.25 2.5 3.58579 2.5 4C2.5 4.41421 2.83579 4.75 3.25 4.75Z" fill="#fa9dae"></path> <path d="M3.25 8.75C3.66421 8.75 4 8.41421 4 8C4 7.58579 3.66421 7.25 3.25 7.25C2.83579 7.25 2.5 7.58579 2.5 8C2.5 8.41421 2.83579 8.75 3.25 8.75Z" fill="#fa9dae"></path> <path d="M3.25 12.75C3.66421 12.75 4 12.4142 4 12C4 11.5858 3.66421 11.25 3.25 11.25C2.83579 11.25 2.5 11.5858 2.5 12C2.5 12.4142 2.83579 12.75 3.25 12.75Z" fill="#fa9dae"></path></svg>';
            } else if (data.type == 'chapter') {
                let color = '#83bee9';
                if (data.color) {
                    color = data.color;
                } else if (data.HTMLStyles && data.HTMLStyles.color) {
                    color = `#${data.HTMLStyles.color}`;
                }
                icon.innerHTML = `<svg width="27" height="27" viewBox="-1 -1.5 17 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13 14H3.99998C3.86737 14 3.7402 13.9473 3.64643 13.8535C3.55267 13.7598 3.5 13.6326 3.5 13.5V2.5C3.5 2.3674 3.55267 2.24022 3.64643 2.14645C3.7402 2.05268 3.86737 2 3.99998 2H10.0002L13.5 5.5V13.5C13.5 13.6326 13.4473 13.7598 13.3536 13.8535C13.2598 13.9473 13.1326 14 13 14V14Z" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M10 2V5.5H13.5005" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;
            } else if (data.type == 'custom') {
                icon.innerHTML =
                    '<svg width="27" height="28" viewBox="-1.4 -1.5 14 17" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="6.5" cy="7" r="6" stroke="#896bd7" stroke-width="0.83"></circle></svg>';
            } else if (data.type == 'character') {
                let color = '#a3d0b0';
                if (data.color) {
                    color = data.color;
                ...
                icon.innerHTML = `<svg width="28" height="28" viewBox="0 -0.8 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.5 14C11.8137 14 14.5 11.3137 14.5 8C14.5 4.68629 11.8137 2 8.5 2C5.18629 2 2.5 4.68629 2.5 8C2.5 11.3137 5.18629 14 8.5 14Z" stroke="${color}" stroke-width="0.833333" stroke-miterlimit="10"></path><path d="M8.5 10C9.88071 10 11 8.88071 11 7.5C11 6.11929 9.88071 5 8.5 5C7.11929 5 6 6.11929 6 7.5C6 8.88071 7.11929 10 8.5 10Z" stroke="${color}" stroke-width="0.833333" stroke-miterlimit="10"></path><path d="M4.48743 12.4609C4.86406 11.7204 5.43825 11.0986 6.14644 10.6642C6.85464 10.2299 7.66921 10 8.49998 10C9.33075 10 10.1453 10.2299 10.8535 10.6642C11.5617 11.0985 12.1359 11.7204 12.5125 12.4609" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path</svg>`;
            }

            // if (data.type == 'outline') {
            //     const color = dataNode.data?.HTMLStyles?.color ? `#${dataNode.data?.HTMLStyles?.color}` : '#fa9dae';
            //     icon.innerHTML = `<svg width="29" height="27" viewBox="0 -0.5 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 4H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 8H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 12H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M3.25 4.75C3.66421 4.75 4 4.41421 4 4C4 3.58579 3.66421 3.25 3.25 3.25C2.83579 3.25 2.5 3.58579 2.5 4C2.5 4.41421 2.83579 4.75 3.25 4.75Z" fill="${color}"></path> <path d="M3.25 8.75C3.66421 8.75 4 8.41421 4 8C4 7.58579 3.66421 7.25 3.25 7.25C2.83579 7.25 2.5 7.58579 2.5 8C2.5 8.41421 2.83579 8.75 3.25 8.75Z" fill="${color}"></path> <path d="M3.25 12.75C3.66421 12.75 4 12.4142 4 12C4 11.5858 3.66421 11.25 3.25 11.25C2.83579 11.25 2.5 11.5858 2.5 12C2.5 12.4142 2.83579 12.75 3.25 12.75Z" fill="${color}"></path></svg>`;
            // } else if (data.type == 'chapter') {
            //     const color = dataNode.data?.HTMLStyles?.color ? `#${dataNode.data?.HTMLStyles?.color}` : '#83bee9';
            //     icon.innerHTML = `<svg width="27" height="27" viewBox="-1 -1.5 17 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13 14H3.99998C3.86737 14 3.7402 13.9473 3.64643 13.8535C3.55267 13.7598 3.5 13.6326 3.5 13.5V2.5C3.5 2.3674 3.55267 2.24022 3.64643 2.14645C3.7402 2.05268 3.86737 2 3.99998 2H10.0002L13.5 5.5V13.5C13.5 13.6326 13.4473 13.7598 13.3536 13.8535C13.2598 13.9473 13.1326 14 13 14V14Z" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M10 2V5.5H13.5005" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;
            // } else if (data.type == 'custom') {
            //     const color = dataNode.data?.HTMLStyles?.color ? `#${dataNode.data?.HTMLStyles?.color}` : '#896bd7';
            //     icon.innerHTML = `<svg width="27" height="28" viewBox="-1.4 -1.5 14 17" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="6.5" cy="7" r="6" stroke="${color}" stroke-width="0.83"></circle></svg>`;
            // } else if (data.type == 'character' || data.type == 'storyelement') {
            //     const color = dataNode.data?.HTMLStyles?.color ? `#${dataNode.data?.HTMLStyles?.color}` : '#a3d0b0';
            //     icon.innerHTML = `<svg width="28" height="28" viewBox="0 -0.8 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.5 14C11.8137 14 14.5 11.3137 14.5 8C14.5 4.68629 11.8137 2 8.5 2C5.18629 2 2.5 4.68629 2.5 8C2.5 11.3137 5.18629 14 8.5 14Z" stroke="${color}" stroke-width="0.833333" stroke-miterlimit="10"></path><path d="M8.5 10C9.88071 10 11 8.88071 11 7.5C11 6.11929 9.88071 5 8.5 5C7.11929 5 6 6.11929 6 7.5C6 8.88071 7.11929 10 8.5 10Z" stroke="${color}" stroke-width="0.833333" stroke-miterlimit="10"></path><path d="M4.48743 12.4609C4.86406 11.7204 5.43825 11.0986 6.14644 10.6642C6.85464 10.2299 7.66921 10 8.49998 10C9.33075 10 10.1453 10.2299 10.8535 10.6642C11.5617 11.0985 12.1359 11.7204 12.5125 12.4609" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path</svg>`;
            // }
    ...
    addNodeImport(dataNode, precanvas) {
        ...
        const deleteNode = document.createElement('div');
        (deleteNode.innerHTML = '<i class="fa-thin fa-x"></i>'),
            deleteNode.setAttribute('id', 'node-' + dataNode.id + '-delete'),
            deleteNode.setAttribute('data-id', dataNode.id),
            deleteNode.classList.add('drawflow-delete-node');
        ...
        if (dataNode.hasOwnProperty('data') && dataNode.data.hasOwnProperty('type')) {
            // if (dataNode.data.type == 'outline') {
            //     icon.innerHTML =
            //         '<svg width="29" height="27" viewBox="0 -0.5 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 4H14" stroke="white" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 8H14" stroke="white" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 12H14" stroke="white" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M3.25 4.75C3.66421 4.75 4 4.41421 4 4C4 3.58579 3.66421 3.25 3.25 3.25C2.83579 3.25 2.5 3.58579 2.5 4C2.5 4.41421 2.83579 4.75 3.25 4.75Z" fill="white"></path> <path d="M3.25 8.75C3.66421 8.75 4 8.41421 4 8C4 7.58579 3.66421 7.25 3.25 7.25C2.83579 7.25 2.5 7.58579 2.5 8C2.5 8.41421 2.83579 8.75 3.25 8.75Z" fill="white"></path> <path d="M3.25 12.75C3.66421 12.75 4 12.4142 4 12C4 11.5858 3.66421 11.25 3.25 11.25C2.83579 11.25 2.5 11.5858 2.5 12C2.5 12.4142 2.83579 12.75 3.25 12.75Z" fill="white"></path></svg>';
            // } else if (dataNode.data.type == 'chapter') {
            //     icon.innerHTML =
            //         '<svg width="27" height="27" viewBox="-1 -1.5 17 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13 14H3.99998C3.86737 14 3.7402 13.9473 3.64643 13.8535C3.55267 13.7598 3.5 13.6326 3.5 13.5V2.5C3.5 2.3674 3.55267 2.24022 3.64643 2.14645C3.7402 2.05268 3.86737 2 3.99998 2H10.0002L13.5 5.5V13.5C13.5 13.6326 13.4473 13.7598 13.3536 13.8535C13.2598 13.9473 13.1326 14 13 14V14Z" stroke="white" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M10 2V5.5H13.5005" stroke="white" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path></svg>';
            // } else if (dataNode.data.type == 'custom') {
            //     icon.innerHTML =
            //         '<svg width="27" height="28" viewBox="-1.4 -1.5 14 17" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="6.5" cy="7" r="6" stroke="white" stroke-width="0.83"></circle></svg>';
            // } else if (dataNode.data.type == 'character' || dataNode.data.type == 'storyelement') {
            //     icon.innerHTML =
            //         '<svg width="28" height="28" viewBox="0 -0.8 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.5 14C11.8137 14 14.5 11.3137 14.5 8C14.5 4.68629 11.8137 2 8.5 2C5.18629 2 2.5 4.68629 2.5 8C2.5 11.3137 5.18629 14 8.5 14Z" stroke="white" stroke-width="0.833333" stroke-miterlimit="10"></path> <path d="M8.5 10C9.88071 10 11 8.88071 11 7.5C11 6.11929 9.88071 5 8.5 5C7.11929 5 6 6.11929 6 7.5C6 8.88071 7.11929 10 8.5 10Z" stroke="white" stroke-width="0.833333" stroke-miterlimit="10"></path> <path d="M4.48743 12.4609C4.86406 11.7204 5.43825 11.0986 6.14644 10.6642C6.85464 10.2299 7.66921 10 8.49998 10C9.33075 10 10.1453 10.2299 10.8535 10.6642C11.5617 11.0985 12.1359 11.7204 12.5125 12.4609" stroke="white" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path></svg>';
            // }

            if (dataNode.data.type == 'outline') {
                const color =
                    dataNode.data.HTMLStyles && dataNode.data.HTMLStyles.color ? `${dataNode.data.HTMLStyles && dataNode.data.HTMLStyles.color}` : '#fa9dae';
                icon.innerHTML = `<svg width="29" height="27" viewBox="0 -0.5 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 4H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 8H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 12H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M3.25 4.75C3.66421 4.75 4 4.41421 4 4C4 3.58579 3.66421 3.25 3.25 3.25C2.83579 3.25 2.5 3.58579 2.5 4C2.5 4.41421 2.83579 4.75 3.25 4.75Z" fill="${color}"></path> <path d="M3.25 8.75C3.66421 8.75 4 8.41421 4 8C4 7.58579 3.66421 7.25 3.25 7.25C2.83579 7.25 2.5 7.58579 2.5 8C2.5 8.41421 2.83579 8.75 3.25 8.75Z" fill="${color}"></path> <path d="M3.25 12.75C3.66421 12.75 4 12.4142 4 12C4 11.5858 3.66421 11.25 3.25 11.25C2.83579 11.25 2.5 11.5858 2.5 12C2.5 12.4142 2.83579 12.75 3.25 12.75Z" fill="${color}"></path></svg>`;
            } else if (dataNode.data.type == 'chapter') {
                const color = dataNode.data.color ? `${dataNode.data.color}` : '#83bee9';
                icon.innerHTML = `<svg width="27" height="27" viewBox="-1 -1.5 17 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13 14H3.99998C3.86737 14 3.7402 13.9473 3.64643 13.8535C3.55267 13.7598 3.5 13.6326 3.5 13.5V2.5C3.5 2.3674 3.55267 2.24022 3.64643 2.14645C3.7402 2.05268 3.86737 2 3.99998 2H10.0002L13.5 5.5V13.5C13.5 13.6326 13.4473 13.7598 13.3536 13.8535C13.2598 13.9473 13.1326 14 13 14V14Z" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M10 2V5.5H13.5005" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;
            } else if (dataNode.data.type == 'custom') {
                let color = '#896bd7';
                if (dataNode.data.color) {
                    color = dataNode.data.color;
                } else if (dataNode.data.HTMLStyles && dataNode.data.HTMLStyles.color) {
                    color = `#${dataNode.data.HTMLStyles.color}`;
                }
                icon.innerHTML = `<svg width="27" height="28" viewBox="-1.4 -1.5 14 17" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="6.5" cy="7" r="6" stroke="${color}" stroke-width="0.83"></circle></svg>`;
            } else if (dataNode.data.type == 'character' || dataNode.data.type == 'storyelement') {
                const color = dataNode.data.color ? `${dataNode.data.color}` : '#a3d0b0';
                icon.innerHTML = `<svg width="28" height="28" viewBox="0 -0.8 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.5 14C11.8137 14 14.5 11.3137 14.5 8C14.5 4.68629 11.8137 2 8.5 2C5.18629 2 2.5 4.68629 2.5 8C2.5 11.3137 5.18629 14 8.5 14Z" stroke="${color}" stroke-width="0.833333" stroke-miterlimit="10"></path><path d="M8.5 10C9.88071 10 11 8.88071 11 7.5C11 6.11929 9.88071 5 8.5 5C7.11929 5 6 6.11929 6 7.5C6 8.88071 7.11929 10 8.5 10Z" stroke="${color}" stroke-width="0.833333" stroke-miterlimit="10"></path><path d="M4.48743 12.4609C4.86406 11.7204 5.43825 11.0986 6.14644 10.6642C6.85464 10.2299 7.66921 10 8.49998 10C9.33075 10 10.1453 10.2299 10.8535 10.6642C11.5617 11.0985 12.1359 11.7204 12.5125 12.4609" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path</svg>`;
            }
            ...
            function (input_item) {
                ...
                Object.keys(dataNode.inputs[input_item].connections).map(
                    function (output_item) {
                        ...
                        const outputConnectionIndex = this.currentContent.drawflow.drawflow[this.currentContent.module].data[outputNodeId].outputs[
                            outputClass
                        ].connections.findIndex((conn) => conn.node == dataNode.id && conn.output == input_item);

                        if (outputConnectionIndex != null && outputConnectionIndex != -1) {
                            const outputConnection =
                                this.currentContent.drawflow.drawflow[this.currentContent.module].data[outputNodeId].outputs[outputClass].connections[
                                    outputConnectionIndex
                                ];
                            if (outputConnection.hasOwnProperty('labelText') && outputConnection.labelText != null) {
                                rotation = outputConnection.labelRotation;
                                this.currentContent.addLabelTextImport(
                                    connection,
                                    outputConnection.labelText,
                                    outputNodeId,
                                    outputClass,
                                    dataNode.id,
                                    input_item,
    ...
    updateNodeDataFromId(id, data, html = null) {
        ...
        if (this.module === moduleName) {
            ...
            if (data.hasOwnProperty('type')) {
                ...
                if (data.type == 'outline') {
                    const color = data.color ? `${data.color}` : '#fa9dae';
                    icon.innerHTML = `<svg width="29" height="27" viewBox="0 -0.5 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 4H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 8H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M6.00037 12H14" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M3.25 4.75C3.66421 4.75 4 4.41421 4 4C4 3.58579 3.66421 3.25 3.25 3.25C2.83579 3.25 2.5 3.58579 2.5 4C2.5 4.41421 2.83579 4.75 3.25 4.75Z" fill="${color}"></path> <path d="M3.25 8.75C3.66421 8.75 4 8.41421 4 8C4 7.58579 3.66421 7.25 3.25 7.25C2.83579 7.25 2.5 7.58579 2.5 8C2.5 8.41421 2.83579 8.75 3.25 8.75Z" fill="${color}"></path> <path d="M3.25 12.75C3.66421 12.75 4 12.4142 4 12C4 11.5858 3.66421 11.25 3.25 11.25C2.83579 11.25 2.5 11.5858 2.5 12C2.5 12.4142 2.83579 12.75 3.25 12.75Z" fill="${color}"></path></svg>`;
                } else if (data.type == 'chapter') {
                    const color = data.color ? `${data.color}` : '#83bee9';
                    icon.innerHTML = `<svg width="27" height="27" viewBox="-1 -1.5 17 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13 14H3.99998C3.86737 14 3.7402 13.9473 3.64643 13.8535C3.55267 13.7598 3.5 13.6326 3.5 13.5V2.5C3.5 2.3674 3.55267 2.24022 3.64643 2.14645C3.7402 2.05268 3.86737 2 3.99998 2H10.0002L13.5 5.5V13.5C13.5 13.6326 13.4473 13.7598 13.3536 13.8535C13.2598 13.9473 13.1326 14 13 14V14Z" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M10 2V5.5H13.5005" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;
                } else if (data.type == 'custom') {
                    const color = data.color ? `${data.color}` : '#896bd7';
                    icon.innerHTML = `<svg width="27" height="28" viewBox="-1.4 -1.5 14 17" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="6.5" cy="7" r="6" stroke="${color}" stroke-width="0.83"></circle></svg>`;
                } else if (data.type == 'character' || data.type == 'storyelement') {
                    const color = data.color ? `${data.color}` : '#a3d0b0';
                    icon.innerHTML = `<svg width="28" height="28" viewBox="0 -0.8 17 17" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.5 14C11.8137 14 14.5 11.3137 14.5 8C14.5 4.68629 11.8137 2 8.5 2C5.18629 2 2.5 4.68629 2.5 8C2.5 11.3137 5.18629 14 8.5 14Z" stroke="${color}" stroke-width="0.833333" stroke-miterlimit="10"></path><path d="M8.5 10C9.88071 10 11 8.88071 11 7.5C11 6.11929 9.88071 5 8.5 5C7.11929 5 6 6.11929 6 7.5C6 8.88071 7.11929 10 8.5 10Z" stroke="${color}" stroke-width="0.833333" stroke-miterlimit="10"></path><path d="M4.48743 12.4609C4.86406 11.7204 5.43825 11.0986 6.14644 10.6642C6.85464 10.2299 7.66921 10 8.49998 10C9.33075 10 10.1453 10.2299 10.8535 10.6642C11.5617 11.0985 12.1359 11.7204 12.5125 12.4609" stroke="${color}" stroke-width="0.833333" stroke-linecap="round" stroke-linejoin="round"></path</svg>`;
                }
            }
